...
def get_report_files():
    ...
    if latest_log and latest_log.executive_summary_path:
        if os.path.exists(latest_log.executive_summary_path):
             exec_summary_report = f"/sql_scanner/download_pdf?target={target}&type=executive" if target else "/sql_scanner/download_pdf?type=executive"
    
    # Fallback to disk check if DB is out of sync
    if not exec_summary_report:
        exec_path = report_manager.find_latest_report(user_dir, "sql_scanner", target=target, extension="pdf")
        if exec_path:
...
@sql_scanner_bp.route('/trigger_executive_summary', methods=['POST'])
@login_required
def trigger_executive_summary():
    """Triggers the AI Executive Brief generation for the SQL injection report."""
    data = request.get_json() or {}
    ...
    report_path = report_manager.find_latest_report(user_dir, "sql_scanner", target=target, extension="pdf")
    ...
    success, result = generate_executive_summary(
        ...
        target=target,
        tool_name="SQL Injection Vulnerability Audit (SQLMap)"
    )
    
    if success:
        download_url = f"/sql_scanner/download_pdf?target={target}&type=executive" if target else "/sql_scanner/download_pdf?type=executive"
        return jsonify({
            "status": "success",
