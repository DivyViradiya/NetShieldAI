document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleScanButtonClick() {
        ...
        if (!targetUrl) {
            appendLog('[!] Error: Target API Base URL is required.');
            toggleTerminal();
            return;
        }
        if (!definitionUrl) {
            appendLog('[!] Error: OpenAPI Definition URL is required.');
            toggleTerminal();
            return;
        }
    ...
    function setupLogStream() {
        ...
        logEventSource.onmessage = function(event) {
            ...
            if (lowerData.includes("scan complete") || lowerData.includes("api scan complete") || event.data.includes("SYSTEM_EVENT: READY_FOR_ANALYSIS")) { 
    ...
    async function fetchAndDisplayResults() {
        ...
        findingsList.innerHTML = `<div style="text-align:center; padding: 4rem 1rem; color: var(--neo-text-muted); font-family: var(--font-mono); font-size: 0.75rem;">LOADING SCAN DATA...</div>`;

        try {
            const response = await fetch(RESULTS_ENDPOINT);
            if (!response.ok) {
                if (response.status === 404) {
                    findingsList.innerHTML = `<div style="text-align:center; padding: 4rem 1rem; color: var(--neo-text-muted); font-family: var(--font-mono); font-size: 0.75rem;">NO SCAN DATA FOUND. START A NEW SCAN.</div>`;
                    return;
                }
            ...
            if (result.status === 'success' && result.data) {
                ...
                findingsList.innerHTML = `<div style="text-align:center; padding: 4rem 1rem; color: var(--neo-text-muted); font-family: var(--font-mono); font-size: 0.75rem;">NO RESULTS AVAILABLE.</div>`;
            }
        } catch (error) {
            console.error(error);
            findingsList.innerHTML = `<div style="text-align:center; padding: 4rem 1rem; color: var(--neo-red); font-family: var(--font-mono); font-size: 0.75rem;">ERROR LOADING DATA.</div>`;
        } finally {
            checkReportStatus();
        }
    }
    ...
    function updateSummaryDisplay(summary, targetUrl) {
        if (!summary) return;
        if (lastScannedUrlDisplay && targetUrl) lastScannedUrlDisplay.textContent = targetUrl;
    ...
    function createFindingCard(finding) {
        ...
        const priorityBadge = rawPriority.split(' ')[0];
        const priorityColor = priorityBadge === 'P0' ? 'var(--neo-red)' : (priorityBadge === 'P1' ? 'var(--neo-amber)' : 'var(--neo-blue)');

        const tctrScore = typeof finding.tctr_priority === 'number' ? finding.tctr_priority.toFixed(4) : (finding.tctr_priority || '—');
        const baseScore = typeof finding.base_score === 'number' ? finding.base_score.toFixed(1) : (finding.base_score || '—');
            ...
            <div class="finding-details">
                <div class="details-content">
                    ...
                    <div class="detail-section">
                        ...
                        <div class="detail-text" style="border-left: 2px solid var(--neo-green); padding-left: 0.75rem;">${finding.solution}</div>
                    </div>` : ''}

                    <div class="detail-section" style="background: rgba(0,0,0,0.2);">
                        <div class="flex justify-between items-center mb-2">
                            <span class="detail-label">TCTR Intelligence</span>
                            <span class="badge-pill" style="background: ${priorityColor}22; border-color: ${priorityColor}44; color: ${priorityColor};">${priorityBadge}</span>
                        </div>
                        <div class="intel-row" style="padding: 0.25rem 0; border: none;">
                            <span class="intel-label" style="font-size: 0.6rem;">TCTR Score</span>
                            <span class="intel-val">${tctrScore}</span>
                        </div>
                        <div class="intel-row" style="padding: 0.25rem 0; border: none;">
                            <span class="intel-label" style="font-size: 0.6rem;">Base Score</span>
                            <span class="intel-val">${baseScore}</span>
                        </div>
                        <div class="intel-row" style="padding: 0.25rem 0; border: none;">
                    ...
                    <div class="detail-section">
                        ...
                        <div class="detail-text-mono">${finding.evidence}</div>
                    </div>` : ''}
                </div>
            </div>
        `;
    ...
    async function analyzeReport(llmMode) {
        ...
        try {
            ...
            response = await fetch(`${CHATBOT_REDIRECT_URL}/scanner_analysis`, {
                ...
                headers: { 'Content-Type': 'application/json', 'X-CSRFToken': csrfToken },
                body: JSON.stringify({ 
                    llm_mode: llmMode, scanner_type: "api", target: data.target, force_new_session: true 
                })
            });
    ...
    if (apiHistoryBtn && historyModal && closeHistoryModal) {
        apiHistoryBtn.addEventListener('click', () => { historyModal.classList.remove('hidden'); fetchHistory(); });
    ...
    async function checkScanStatus() {
        try {
            ...
            if (data.status === 'success' && data.is_running) {
                ...
                if (lastScannedUrlDisplay) lastScannedUrlDisplay.textContent = data.target;
                appendLog(`[*] Detected active API scan on ${data.target}. Attaching stream...`);
                setupLogStream();
            }
    ...
    // --- On Load ---
    setTimeout(() => appendLog('System Ready. Initializing API Security Scanner interface...'), 100);
    checkReportStatus();
    fetchAndDisplayResults();
    checkScanStatus();
    setupLogStream();
